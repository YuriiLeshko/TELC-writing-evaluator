"""TELC B2 relevance pre-check implementation.

This module runs the first evaluation pre-check that verifies only:
1) topic mismatch (Thema verfehlt)
2) situation mismatch (Situierung verfehlt)

It does not perform scoring and does not evaluate grammar, structure, or key
point fulfillment. It only validates LLM JSON output against Pydantic schemas.
"""
...
from backend.evaluation.schemas import RelevanceCheckResult, WritingEvaluationInput
...
async def check_relevance(
    ...
    return RelevanceCheckResult.model_validate(raw_result)
...
if __name__ == "__main__":
    ...
    async def _smoke_test() -> None:
        ...
        result = await check_relevance(llm_client=llm_client, input_data=input_data)
